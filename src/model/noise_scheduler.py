...
class GaussianDiffusion(nn.Module):
    ...
    # Returns x_(t-1) given x_0, x_t, and t.
    def q_posterior(self, x_start: torch.tensor, x_t: torch.tensor, t: torch.tensor) -> tuple[torch.tensor,
        ...
        ## Copy and pasted from `Imagen.py` from the same tutorial.
        # Noise for sampling
        b = x_start.shape[0]
        ...
        nonzero_mask = (1 - is_last_sampling_timestep.float()).reshape(b, *((1,) * (len(x_start.shape) - 1)))
        ...
        return posterior_mean + nonzero_mask * (0.5 * posterior_log_variance_clipped).exp() * noise

    # Creates a noised image from the original image and a timestep value. Leave noise as None.
    ...
    # Predict original image (x_0) from current image (x_t), current timestep (t), and predicted noise (noise).
